...
@Description:
ExShipper project is to integrate Google Cloud Technology to improve global logistics services
'''
__author__ = 'Alan Tai'
...
#                     log_spearnet_customer_package_info = data
        ...
        # packages status are apex, sfo, taoyuan international airport, and suda
            ...
            # query package information (packages' status == spearnet or exshipper)
            spearnet_customer_package_info_log = SpearnetPackagesInfo.query(SpearnetPackagesInfo.package_status == 'delivered')
            # pass clients informations
            clients_info = ClientsInfo().query()
            template_values.update({'spearnet_customer_package_info_log': spearnet_customer_package_info_log, 'clients_info':clients_info, 'dispatch_token':dispatch_token})
        ...
        # tw custom entry packages; package status exshipper
            ...
            tw_custom_entry_packages_info_log = TWCustomEntryInfo.query(ndb.OR(TWCustomEntryInfo.package_status == 'NA',
                                                                               TWCustomEntryInfo.package_status == 'exshipper'))
        ...
        # page of handling exshipper pre-alert
            ...
            pre_alert_package_entity = TWCustomEntryInfo.query(TWCustomEntryInfo.package_status == 'exshipper').get()
            pre_alert_packages_list = TWCustomEntryInfo.query(TWCustomEntryInfo.package_status == 'exshipper')
            
            # check if the packages which will be shipped have different mawb, flight number, or flight date 
            if(pre_alert_package_entity != None and pre_alert_packages_list != None):
                ...
                for package_info in pre_alert_packages_list:
                    if(pre_alert_package_entity.mawb != package_info.mawb):
                        mawb_response = mawb_response + package_info.mawb
                        has_inconsistent_data = 'yes'
                    if(pre_alert_package_entity.flight_number != package_info.flight_number):
                        flight_number_response = flight_number_response + package_info.flight_number
                        has_inconsistent_data = 'yes'
                    if(pre_alert_package_entity.flight_date != package_info.flight_date):
                        flight_date_response = flight_date_response + package_info.flight_date.strftime("%Y-%m-%d")
                        has_inconsistent_data = 'yes'
                ...
                flight_number = pre_alert_package_entity.flight_number
                flight_date = pre_alert_package_entity.flight_date
                mawb = pre_alert_package_entity.mawb
                sender = pre_alert_package_entity.sender
                receiver = pre_alert_package_entity.receiver
                template_values.update({'flight_number':flight_number, 'flight_date':flight_date, 'mawb':mawb, 'sender':sender, 'receiver':receiver})
                
            template_values.update({'pre_alert_packages_list':pre_alert_packages_list})
                ...
                if(json_obj_package_status != 'NA'):
                    for key in json_obj_package_status:
                        ...
                        if(notification_email_entity != None and mail.is_email_valid(notification_email_entity.email)):
                            ...
                            #email customer the notification of package status
                            #html format test
                            email_package_status_notification = mail.EmailMessage(sender = ' ExShipper <winever.tw@gmail.com>', subject = 'Packages Status Notification')
...
#general clients packages tracking handler
...
class ExShipperPackagesInfoLogMigrationHandler(webapp2.RequestHandler):
    def get(self):
        ...
        #tw custom entry packages info migration
...
#delete email notification once the package is delivered  
