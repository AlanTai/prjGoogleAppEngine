...
$(document).ready(
		function() {
			 ...
			 * 
			 * $($(this).data("tooltip")).css({ left: e.pageX + 1, top: e.pageY +
			 * 1 }).stop().show(100); }, function() {
			 * $($(this).data("tooltip")).hide(); });
			 */
			// end of mousehover opoup txt
			
			// toggle button for information content
			$('#container_toggle_btn').click(
					function() {
						$('#container').slideToggle(
								800,
								function() {
									var img_src = $('#container_toggle_btn')
											.attr('src');
									if (img_src === '/img/arrow_down_new.png') {
										$('#container_toggle_btn').attr('src',
												'/img/arrow_up_new.png');
									} else {
										$('#container_toggle_btn').attr('src',
												'/img/arrow_down_new.png');
									}
			...
			$('#question_answer> p:first-child').css({
				'color' : '#f00',
				'border-bottom' : 'solid 1px #f00'
			});
			$('#question_answer> p').on('click', show_question_answer);
			// end of question&answer

			// logistics
			$('#logistics_process_main> section#酒款訂購').css({
				'display':'block'
			});
			$('#logistics_process_title_bar> div:first-child> p').css({
				'color':'#f00'
			});
			
			$('#logistics_process_title_bar> div').on('click',
					show_logistics_process);
			// end of logistics
...
function show_question_answer() {
	...
	$('#question_answer> p').css({
		'border-bottom' : 'solid 1px transparent'
	});
	$('#question_answer> p> span').css({
		'color' : '#644901'
	});
	$(this).css({
		'border-bottom' : 'solid 1px #f00'
...
// logistics process
function show_logistics_process() {
	var section_id = $(this).attr('title');
	$('#logistics_process_title_bar> div> p').css({
		'color' : '#644901'
	});
	$(this).find('p').css({
		'color':'#f00'
	...
	$('#logistics_process_title_bar> div').hover(function() {
		$(this).css({
			'color':'#f00'
		});
	}, function() {
	...
	$('#logistics_process_main> section').css({
		'display':'none'
	});
	$('#logistics_process_main> section#'+section_id).css({
		'display':'block'
	});
}
// end of logistics process
...
function toggle_height() {
	...
	if (page_tag === 'link_page') {
		 ...
		 * $('#logistics_process').animate({paddingTop: '10px'},100); } else{
		 * $('#logistics_process').animate({paddingTop: '20px'},100); }
		 */
	} else {
		if ((container_display === 'block')
				|| (info_area_heigh > 35 || contact_area_heigh > 35)) {
			$('#logistics_process').animate({
				paddingTop : '10px'
			}, 1000);
